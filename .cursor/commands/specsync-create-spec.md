...
1. Remove `--minimal` wherever it appears and remember that minimal mode was
   requested. Preserve the complete remaining argument text.
2. Classify the complete remaining text before choosing a module name:
...
5. Open the newly created `specs/<module-name>/<module-name>.spec.md` and fill
   in every required section: `Purpose`, `Public API`, `Invariants`,
   `Behavioral Examples`, `Error Cases`, `Dependencies`, and `Change Log`. If a free-text
