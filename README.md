...
> **macOS and Linux.** `attest`/`AttestKit` builds and is CI-tested on both macOS 13+ and Linux (see the upstream repo's `build-test-macos` and `build-test-linux` jobs), so this plugin supports both as well. Windows is out of scope.
>
